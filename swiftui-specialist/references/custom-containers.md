...
| Adding a parameter so the caller can supply a row modifier the container must sequence | If the container has to apply it *after* its own row modifier, the caller can't supply it — a caller closure only builds the inner label. This is the signal to invert: let the container own the modifier and take a `Binding` to a request value the row writes. See `dataflow.md` § "A Binding is also an upward intent channel" |

...
- That branch is not only a cost. MEASURED (macOS 27): a `_ConditionalContent` inside a row's subtree **defeats the drag lift** once a container-level reorder modifier is installed — that one is a hard ban, because the lift has no way to re-assert. A branch flip can also re-root a *sibling* subtree and discard state the diff never touched (`scenes.md` § Re-rooting); there the branch is rarely the right target — fix the consumer that failed to re-assert. Treat the ternary as the default and a state-driven branch as the exception that has to be justified.

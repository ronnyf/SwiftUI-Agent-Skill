...
- There is **no native window-close hook** (no `windowWillClose`-equivalent SwiftUI modifier through current macOS), **but the root view's `onDisappear` *does* fire on macOS window close** — this is the simpler hook for window-scoped teardown (programmatic / confirmation dismissal surface: `dismissWindow` / `DismissWindowAction` / `windowDismissBehavior` / `dismissalConfirmationDialog`).

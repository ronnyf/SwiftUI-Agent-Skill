...
**Issue:**
Projects that provide an initial value for a `@State` variable and try to assign its value again in the init, before all stored properties are assigned, will encounter errors like:
...
**Fix:**
 Reorder assignments in init so all non-`@State` stored properties are initialized before any `@State` properties and avoid initializing the value of state twice.
...
If the intent is to set the value in the `init` function, avoid setting a default value on the property:
```swift
struct ContentView: View {
